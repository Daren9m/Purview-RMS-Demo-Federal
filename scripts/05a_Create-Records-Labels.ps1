...
foreach ($item in $plan) {
  ...
  if ($item.RetentionDurationDays) {
    $duration = "$($item.RetentionDurationDays) Days"
  } elseif ($item.RetentionDurationYears -lt 0) {
    $duration = "Unlimited"
  } else {
    $duration = "$($item.RetentionDurationYears) Years"
  }
...
# Examples: mark FOIA/Contracts as record-required
